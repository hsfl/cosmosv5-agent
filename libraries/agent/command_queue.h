 ...
 * \brief Agent command queue with scheduling and event integration.
 * \ingroup cosmos_agent_framework
 * \defgroup cosmos_command_queue Command Queue
 *
 * Implements the priority queue that holds pending agent commands together
 * with their scheduled execution times and associated Event metadata. Provides
 * thread-safe enqueue/dequeue and integrates with the scheduler for time-based
 * command dispatch.
 */
