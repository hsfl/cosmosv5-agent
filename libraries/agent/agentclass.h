...
/*!
 * \file agentclass.h
 * \brief COSMOS agent: heartbeat, request interface, namespace, and CPU monitoring.
 * \ingroup cosmos_agent_framework
 * \defgroup cosmos_agentclass Agent Class
 *
 * The central class for every COSMOS software agent. Initialises the shared
 * namespace, starts the heartbeat broadcast, opens the request socket, and
 * provides the run loop. Agents subclass or instantiate AgentClass to gain
 * automatic discovery, remote procedure call, and telemetry publishing without
 * boilerplate.
 */
