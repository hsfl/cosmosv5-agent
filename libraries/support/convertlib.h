...
/*!
 * \file convertlib.h
 * \brief Coordinate frame conversions: ECI, ECEF, geodetic, topocentric, orbital.
 * \ingroup cosmos_agent_namespace
 * \defgroup cosmos_convertlib Convert
 *
 * Provides a comprehensive set of coordinate transformation functions covering
 * Earth-Centred Inertial (ECI), Earth-Centred Earth-Fixed (ECEF), geodetic
 * (LLA), topocentric (AZ/EL), and orbital (Keplerian) frames. Also handles
 * precession, nutation, polar wander, and GMST for high-fidelity conversions.
 * Central to both the orbit propagator and the antenna-pointing subsystem.
 */
...
namespace Cosmos {
    namespace Convert {
        ...
        //! \brief Stamp all position frame UTC fields with the given epoch. Call before pos_set_xxx() when the locstruc was loaded without timestamps (e.g., static targets). Does not touch extra.utc (managed by pos_extra's cache).
