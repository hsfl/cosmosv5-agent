 ...
 * \brief High-level file transfer state machine using agentclass.
 * \ingroup cosmos_agent_messaging
 * \defgroup cosmos_transferclass Transfer Class
 *
 * Implements the COSMOS file-transfer protocol state machine on top of
 * transferlib. Manages the queue of outgoing files, handles incoming chunk
 * requests and acknowledgements, and drives retransmission. Used by the
 * file-transfer agent (agent_file) and modules that need reliable file
 * delivery.
 */
