 ...
 * \brief Route packets to agents; decode and dispatch commands.
 * \ingroup cosmos_agent_messaging
 * \defgroup cosmos_packethandler Packet Handler
 *
 * Implements a packet-dispatch engine that matches incoming PacketComm packets
 * by type and destination, then calls the registered handler callback. Handles
 * command decoding, response encoding, and error reporting. Used by agentclass
 * and the ground-station uplink path.
 */
